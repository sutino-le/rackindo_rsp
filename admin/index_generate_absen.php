...
$periode_awal = date('Y-m-d', strtotime('-2 days', strtotime($periode_akhir)));
$mulai_tanggal = date('Y-m-26', strtotime('-1 month', strtotime($periode_akhir)));
...
while (strtotime($periode_awal) <= strtotime($periode_akhir)) {
    ...
    $data_karyawan = mysql_query("SELECT * FROM karyawan ORDER BY karyawan_finger ASC");
...
window.setTimeout(function() {
    document.location = 'index_download_finger.php';
}, 1000);
</script>
<?php
